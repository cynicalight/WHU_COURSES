...
class Jsteg:
    ...
    def set_sequence_after_dct(self, sequence_after_dct):
        ...
        self.available_info_len = len(
            # 使用列表推导式筛选出所有不等于-1、0、1的DCT系数
            [i for i in self.sequence_after_dct if i not in (-1, 1, 0)])
    ...
    def write(self, info):
        ...
        # 假设系统决定用4位来存储长度信息   
        # 3的二进制是 '11'，补齐4位就是 '0011'
        # 所以info变成了: [0, 0, 1, 1, 1, 0, 1]
        # 前4位(0011)表示长度，后3位(101)是原始信息
        info_len = len(info)
        info_index = 0
        im_index = 0
        while True:
