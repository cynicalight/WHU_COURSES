...
plt.plot(x_values, y_values, label='Reconstructed Curve for Equiarc-length Sampling')
q3_x.pic()
plt.xlabel('x')
plt.ylabel('y')
...
plt.grid(True)
# 保存图形到文件
plt.savefig(f'{dir_pic}q3_arc.png')
plt.show()
plt.clf()


# 生成示例数据：曲率和相应的曲线路径
x_curvature = np.linspace(0, 10, 100)
y_curvature = np.sin(x_curvature)  # 示例曲率数据
x_path = np.linspace(0, 10, 100)
y_path = np.sin(x_path) + np.random.normal(0, 0.1, 100)  # 示例曲线路径数据

# 神经网络模型训练
scaler = MinMaxScaler()
x_curvature_scaled = scaler.fit_transform(x_curvature.reshape(-1, 1))
y_curvature_scaled = scaler.fit_transform(y_curvature.reshape(-1, 1))
mlp = MLPRegressor(hidden_layer_sizes=(10, 10), max_iter=1000)
mlp.fit(x_curvature_scaled, y_path)

# 使用神经网络进行曲线重构
x_new_scaled = scaler.transform(x_curvature.reshape(-1, 1))
y_path_reconstructed = mlp.predict(x_new_scaled)

# Bezier曲线重构
spline = UnivariateSpline(x_curvature, y_path, k=3)
x_bezier = np.linspace(0, 10, 1000)
y_bezier = spline(x_bezier)

# 绘制结果
plt.figure(figsize=(10, 5))

plt.subplot(1, 2, 1)
plt.plot(x_path, y_path, label='Original Path')
plt.plot(x_new_scaled, y_path_reconstructed,
         label='Reconstructed Path (Neural Network)')
plt.xlabel('X')
plt.ylabel('Y')
plt.title('Neural Network Reconstruction')
plt.legend()

plt.subplot(1, 2, 2)
plt.plot(x_path, y_path, label='Original Path')
plt.plot(x_bezier, y_bezier, label='Reconstructed Path (Bezier)')
plt.xlabel('X')
plt.ylabel('Y')
plt.title('Bezier Curve Reconstruction')
plt.legend()
