...
# 使用pandas读取数据
data = pd.read_csv("居民消费.csv", encoding='gbk')

# 将数据转换为适合绘图的格式
data_melted = data.melt(id_vars=["指标"], var_name="年份", value_name="指数")
...
# 绘制多个折线图，每个行业一条线
sns.lineplot(data=data_melted, x="年份", y="指数", hue="指标", marker="o")

# 添加标题和标签
plt.title('居民消费变化趋势', fontsize=16)
